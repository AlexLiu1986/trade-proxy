package com.lecheng.trade.service.infos.impl;

import com.lecheng.trade.annotation.HttpRequest;
import com.lecheng.trade.facade.constants.RespCode;
import com.lecheng.trade.facade.dto.infos.faq.GetListResponse;
import com.lecheng.trade.facade.model.Faq;
import com.lecheng.trade.service.BaseServiceImpl;
import com.lecheng.trade.service.infos.FaqService;
import com.lecheng.trade.utils.JsonUtils;
import net.sf.json.JSONArray;
import net.sf.json.JSONObject;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.stereotype.Service;

import java.util.HashMap;
import java.util.List;

/**
 * 功能: 常见问题服务实现
 * 创建: liuchongguang
 * 日期: 2016/12/30 0030 15:16
 * 版本: V1.0
 */
@Service
public class FaqServiceImpl extends BaseServiceImpl implements FaqService {

    /**
     * 日志记录器
     */
    private static Logger logger = LoggerFactory.getLogger(FaqServiceImpl.class);

    /**
     * 获取常见问题列表
     *
     * @return
     */
    @Override
    @HttpRequest(name = "infos/faq/getlist")
    public GetListResponse getList() {
        GetListResponse response = null;
        try {
            HashMap<String, String> paramMap = new HashMap<>();
            JSONObject result = httpClient.doJsonGet(this.httpRequestUrl, paramMap);
            response = new GetListResponse(RespCode.SUCC.getValue(), RespCode.SUCC.getDesc());
            JSONArray arrays = JsonUtils.getJSONArray(result, "FaqList");
            if (arrays != null) {
                List<Faq> faqList = JsonUtils.toBeanList(arrays, Faq.class);
                response.setFaqList(faqList);
            }
        } catch (Exception e) {
            logger.error("查询常见问题列表失败", e);
            response = new GetListResponse(RespCode.FAIL.getValue(), RespCode.FAIL.getDesc());
        }
        return response;
    }
}
